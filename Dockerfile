...
ENV GIT_REPOSITORY=https://github.com/fireice-uk/xmr-stak-rx.git \
    GIT_BRANCH=2.10.7
...
RUN  set -x \
  ...
  && git clone --single-branch --depth 1 --branch $GIT_BRANCH $GIT_REPOSITORY xmr-stak-rx \
  && git -C xmr-stak-rx apply /tmp/donate-level.patch \
  && cd xmr-stak-rx \
  && cmake ${CMAKE_FLAGS} . \
  && make \
...
COPY --from=build /tmp/xmr-stak-rx/bin/* /usr/local/bin/
...
ENTRYPOINT ["/usr/local/bin/xmr-stak-rx"]
